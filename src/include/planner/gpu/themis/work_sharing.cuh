...
struct Task {
    ...
    __forceinline__ __device__ char *GetAttrPtr()
    {
        return ((char *)this) + 16;
    }

    __forceinline__ __device__ bool Pop(int &lvl, int &start, int &end,
                                        int size)
    {
        lvl = lvl_;
        end = end_;
...
struct TaskBook {
    ...
    __device__ Task *GetTaskEntry(unsigned long long length)
    {
        return (Task *)(((char *)this) + 128 + length);
    }

    __device__ Task *AllocTask(int lvl, int &start, int &end, int attr_size,
                               int threshold)
    {
...
struct TaskStack {
    ...
    {
        StackEntry *entries_ =
            (StackEntry *)(((char *)this) + sizeof(TaskStack));
    ...
    __device__ bool Push(Task *task)
    {
        ...
        if (thread_id == 0)
            length_ = base_length + num_to_push;
        if (thread_id == 0 || task)
            __threadfence();
        if (thread_id == 0)
            atomicCAS(&lock_, 1, 0);
    ...
    __device__ char *Pop(int &lvl, int &start, int &end, int size,
                         bool &is_empty_after_pop)
    {
                      ...
                      Themis::WarpsStatus *warp_status)
{
    ...
    if (thread_id == 0) {
        ...
        do {
            if (num_working_warps == num_idle_warps)
                break;
...
__device__ char *Wait(int warp_id, int thread_id, TaskStack *&target_stack,
                      int &lvl, int &start, int &end, int size,
                      TaskBook *taskbook, TaskStack *taskstack,
                      unsigned long long *num_buffered_inodes,
                      ...
                      Themis::WarpsStatus *warp_status)
{
    ...
    if (thread_id == 0) {
        num_idle_warps = warp_status->addIdleWarpNum(num_working_warps) + 1;
        do {
            unsigned long long n_buffered_inodes =
                atomicCAS(num_buffered_inodes, 0, 0);
            if (num_working_warps == num_idle_warps && n_buffered_inodes == 0)
                break;
            int buffer_id = -1;
            Themis::Detection::TwoLvlBitmaps::Get<MIN_NUM_WARPS>(
                warp_id, buffer_id, bit1, bit2, num_idle_warps);
            if (buffer_id > -1) {
                TaskStack *target_stack =
                    (TaskStack *)(((char *)taskstack) + buffer_id * 1024 * 64);
                bool is_empty_after_pop = true;
                attr = target_stack->Pop(lvl, start, end, size,
                                         is_empty_after_pop);
                if (attr) {
                    ...
                    n_buffered_inodes =
                        atomicAdd(num_buffered_inodes,
                                  (unsigned long long)((~num_to_pull) + 1));
                    //printf("n_buffered_inodes: %llu, %llu\n", n_buffered_inodes, num_to_pull);
                    num_idle_warps = warp_status->subIdleWarpNum() - 1;
                    if (!is_empty_after_pop) {
                        Themis::Detection::TwoLvlBitmaps::Set(buffer_id, bit1,
                                                              bit2);
                    }
                    break;
                }
    ...
    //return NULL;
    target_stack =
        (TaskStack *)__shfl_sync(ALL_LANES, (uint64_t)target_stack, 0);
    start = __shfl_sync(ALL_LANES, start, 0);
    end = __shfl_sync(ALL_LANES, end, 0);
    lvl = __shfl_sync(ALL_LANES, lvl, 0);
    return (char *)__shfl_sync(ALL_LANES, (uint64_t)attr, 0);
