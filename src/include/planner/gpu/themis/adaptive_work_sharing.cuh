...
__global__ void krnl_InitStatisticsPerLvl(StatisticsPerLvl *stats,
                                          unsigned int num_warps,
...
__global__ void krnl_InitStatisticsPerLvl(
    StatisticsPerLvl *stats, unsigned int num_warps, int num_inodes_at_zero,
    int depth, int num_total_active_warps, int8_t order_of_case_0,
    int num_warps_of_case_0, int8_t order_of_case_1, int num_warps_of_case_1,
    int8_t order_of_case_2, int num_warps_of_case_2)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void PrintStatisticsPerLvl(StatisticsPerLvl *device_stats,
                           const char *krnl_name, int depth,
                           unsigned int num_warps)
{
    //return;
    StatisticsPerLvl host_stats[128];
    cudaMemcpy(host_stats, device_stats, sizeof(StatisticsPerLvl) * depth,
...
struct LocalLevelAndOrderInfo {
    ...
    __device__ void print(int lvl)
    {
        ...
        printf("LvlAndOrder %d, %d: %d, %d, %d, %d, %d\n", warp_id, lvl,
               num_nodes_at_locally_lowest_lvl, locally_lowest_lvl,
               locally_max_order, globally_lowest_lvl, globally_max_order);
    }
};

__device__ __forceinline__ void FetchLvlAndOrder(
    int thread_id, LocalLevelAndOrderInfo &local_info,
    StatisticsPerLvl *global_stats_per_lvl)
{
    ...
    if (thread_id == 0) {
        ...
        while (locally_lowest_lvl > globally_lowest_lvl) {
            unsigned int num_warps_at_globally_lowest_lvl =
                atomicCAS(&global_stats_per_lvl[globally_lowest_lvl].num_warps,
                          0xFFFFFFFF, 0xFFFFFFFF);
            if (num_warps_at_globally_lowest_lvl > 0)
                break;
            ++globally_lowest_lvl;
        ...
        if (locally_lowest_lvl == globally_lowest_lvl) {
            globally_max_order = (int8_t)atomicCAS(
                &global_stats_per_lvl[globally_lowest_lvl].max_order,
                0xFFFFFFFF, 0xFFFFFFFF);
            local_info.num_warps_which_can_push =
                atomicCAS(&global_stats_per_lvl[globally_lowest_lvl]
                               .sub_num_warps[globally_max_order],
                          0xFFFFFFFF, 0xFFFFFFFF);
            ...
            while (local_info.num_warps_which_can_push == 0) {
                ...
                local_info.num_warps_which_can_push =
                    atomicCAS(&global_stats_per_lvl[globally_lowest_lvl]
                                   .sub_num_warps[new_globally_max_order],
                              0xFFFFFFFF, 0xFFFFFFFF);
            }
            ...
            if (globally_max_order > new_globally_max_order) {
                ...
                atomicMin(&global_stats_per_lvl[globally_lowest_lvl].max_order,
                          new_globally_max_order);
            }
            globally_max_order = new_globally_max_order;
        }
    ...
    local_info.num_warps_which_can_push =
        __shfl_sync(ALL_LANES, local_info.num_warps_which_can_push, 0);
...
__device__ __forceinline__ void FetchLvlAndOrder(
    int thread_id, int &lowest_lvl, int &globally_lowest_lvl,
    unsigned int &num_warps_which_can_push, int8_t &order,
    int8_t &globally_current_max_order, int8_t &globally_max_order,
    int inodes_cnts, StatisticsPerLvl *global_stats_per_lvl, int depth)
{
    unsigned int num_warps_at_globally_lowest_lvl;
    if (thread_id == 0) {
        // Update the globally lowest lvel
        while (true) {
            num_warps_at_globally_lowest_lvl =
                atomicCAS(&global_stats_per_lvl[globally_lowest_lvl].num_warps,
                          0xFFFFFFFF, 0xFFFFFFFF);
            if (num_warps_at_globally_lowest_lvl > 0)
                break;
            if (++globally_lowest_lvl >= depth)
                break;
        }
        // Update the maximum order if lowest_lvl is the globally_lowest_lvl
        if (lowest_lvl == globally_lowest_lvl) {
            globally_current_max_order =
                atomicCAS(&global_stats_per_lvl[globally_lowest_lvl].max_order,
                          0xFFFFFFFF, 0xFFFFFFFF);
            num_warps_which_can_push =
                atomicCAS(&global_stats_per_lvl[globally_lowest_lvl]
                               .sub_num_warps[globally_current_max_order],
                          0xFFFFFFFF, 0xFFFFFFFF);
            if (num_warps_which_can_push == 0) {
                while (--globally_current_max_order >= 0) {
                    num_warps_which_can_push = atomicCAS(
                        &global_stats_per_lvl[globally_lowest_lvl]
                             .sub_num_warps[globally_current_max_order],
                        0xFFFFFFFF, 0xFFFFFFFF);
                    if (num_warps_which_can_push > 0)
                        break;
    ...
    num_warps_which_can_push =
        __shfl_sync(ALL_LANES, num_warps_at_globally_lowest_lvl, lowest_lvl);
    globally_lowest_lvl =
        __shfl_sync(ALL_LANES, globally_lowest_lvl, lowest_lvl);
    globally_current_max_order =
        __shfl_sync(ALL_LANES, globally_current_max_order, lowest_lvl);
}

__device__ __forceinline__ void chooseNextIntervalAfterPush(
    unsigned int &interval, LocalLevelAndOrderInfo &local_info, int num_warps,
    int num_idle_warps, bool is_allowed, unsigned int max_interval)
{
...
__device__ __forceinline__ void chooseNextIntervalAfterPush(
    unsigned int &interval, unsigned int num_warps, unsigned int num_idle_warps,
    bool is_allowed, unsigned int max_interval)
{
    unsigned int num_busy_warps = num_warps - num_idle_warps;
    interval = (32 - __clz(num_busy_warps / num_idle_warps));
}

__device__ bool isPushingAllowed(int thread_id, WarpsStatus *warp_status,
    ...
    if (thread_id == 0) {
        ...
        is_allowed =
            ...
            ((local_info.globally_max_order == local_info.locally_max_order) ||
             ((local_info.globally_max_order ==
               local_info.locally_max_order + 1) &&
              (num_idle_warps >= 2 * local_info.num_warps_which_can_push)));
    }
...
__device__ __forceinline__ void FindIdleWarp(
    int &idle_warp_id, int warp_id, int thread_id, WarpsStatus *warp_status,
    ...
    unsigned long long *bit1, unsigned long long *bit2)
{
    ...
    //return;
    if (thread_id == 0) {
        Detection::TwoLvlBitmaps::Get<MIN_NUM_WARPS>(
            warp_id, idle_warp_id, bit1, bit2, num_idle_warps);
        if (idle_warp_id > -1) {
            num_idle_warps = warp_status->subIdleWarpNum() - 1;
            PushedParts::GetIthStack(gts, size_of_stack_per_warp, idle_warp_id)
                ->TryLock();
        }
    }
    num_idle_warps = __shfl_sync(ALL_LANES, num_idle_warps, 0);
    ...
    Detection::Stack::IdStack *id_stack)
{
    ...
    if (thread_id == 0) {
        ...
        Detection::Stack::Get<MIN_NUM_WARPS>(idle_warp_id, warp_status,
                                             num_idle_warps, id_stack);
        if (idle_warp_id > -1) {
            PushedParts::GetIthStack(gts, size_of_stack_per_warp, idle_warp_id)
                ->TryLock();
        }
...
__forceinline__ __device__ void WaitPushing(
    int warp_id, WarpsStatus *warp_status,
    //unsigned int* global_num_idle_warps,  unsigned int* gpart_ids,
    unsigned int &num_idle_warps, unsigned int &num_working_warps,
    PushedParts::PushedPartsStack *gts, int size_of_stack_per_warp)
{
    PushedParts::PushedPartsStack *stack =
        PushedParts::GetIthStack(gts, size_of_stack_per_warp, warp_id);
    unsigned i = 0;
...
template <uint64_t NUM_WARPS, uint64_t MIN_NUM_WARPS>
__device__ void Wait(int &gpart_id, int &busy_warp_id, int warp_id,
                     int thread_id, int &lowest_lvl, WarpsStatus *warp_status,
                     unsigned int &num_idle_warps,
                     StatisticsPerLvl *global_stats_per_lvl,
                     PushedParts::PushedPartsStack *gts,
                     size_t size_of_stack_per_warp, unsigned long long *bit1,
                     unsigned long long *bit2)
{
    ...
    if (num_idle_warps == num_working_warps)
        return;
    if (gpart_id >= MIN_NUM_WARPS)
        return;
    if (thread_id == 0) {
        if (num_idle_warps != num_working_warps) {
            PushedParts::PushedPartsStack *stack =
                PushedParts::GetIthStack(gts, size_of_stack_per_warp, gpart_id);
            bool locked = stack->TryLock();
            Detection::TwoLvlBitmaps::Set(gpart_id, bit1, bit2);
            WaitPushing<NUM_WARPS, MIN_NUM_WARPS>(
                gpart_id, warp_status, num_idle_warps, num_working_warps, gts,
                     ...
                     Detection::Stack::IdStack *id_stack)
{
    ...
    if (thread_id == 0) {
        ...
        Detection::Stack::Set<NUM_WARPS>(gpart_id, warp_status, num_idle_warps,
                                         id_stack);
        //printf("~wait %d, %d, %d:q\n", gpart_id, num_idle_warps, num_working_warps);
        WaitPushing<NUM_WARPS, MIN_NUM_WARPS>(gpart_id, warp_status,
                                              num_idle_warps, num_working_warps,
                                              gts, size_of_stack_per_warp);
        if (num_idle_warps != num_working_warps) {
            lowest_lvl = stack->Top()->lvl;  //(warp_id, thread_id)->lvl;
            busy_warp_id = gpart_id;
        }
...
__device__ __forceinline__ void UpdateWorkloadSizeOfIdleWarpAfterPush(
    int thread_id, int lvl_to_push, int num_to_push,
    StatisticsPerLvl *global_stats_per_lvl)
{
    if (thread_id == 0) {
        int8_t new_order = CalculateOrder(num_to_push);
        atomicAdd(&global_stats_per_lvl[lvl_to_push].sub_num_warps[new_order],
                  1);
        atomicAdd(&global_stats_per_lvl[lvl_to_push].num_warps, 1);
    }
}

__device__ __forceinline__ void UpdateWorkloadSizeOfBusyWarpAfterPush(
    int thread_id, unsigned mask_1, int new_num_nodes_at_locally_lowest_lvl,
    int new_local_lowest_lvl, int8_t new_local_max_order,
    LocalLevelAndOrderInfo &local_info, StatisticsPerLvl *global_stats_per_lvl)
{
    if (thread_id == 0) {
        if (local_info.locally_lowest_lvl != new_local_lowest_lvl) {
            if (mask_1 != 0) {
                atomicAdd(&global_stats_per_lvl[new_local_lowest_lvl]
                               .sub_num_warps[new_local_max_order],
                          1);
                atomicAdd(&global_stats_per_lvl[new_local_lowest_lvl].num_warps,
                          1);
            }
        }
        else {
            atomicAdd(&global_stats_per_lvl[local_info.locally_lowest_lvl]
                           .sub_num_warps[new_local_max_order],
                      1);
        }
        atomicSub(&global_stats_per_lvl[local_info.locally_lowest_lvl]
                       .sub_num_warps[local_info.locally_max_order],
                  1);
        if (local_info.locally_lowest_lvl != new_local_lowest_lvl)
            atomicSub(
                &global_stats_per_lvl[local_info.locally_lowest_lvl].num_warps,
                1);
    }

    local_info.locally_lowest_lvl = new_local_lowest_lvl;
    local_info.locally_max_order = new_local_max_order;
    local_info.num_nodes_at_locally_lowest_lvl =
        new_num_nodes_at_locally_lowest_lvl;
}

__device__ __forceinline__ void UpdateWorkloadSizeAtZeroLvl(
    int thread_id, unsigned &loop, LocalLevelAndOrderInfo &local_info,
    StatisticsPerLvl *global_stats_per_lvl)
{
    local_info.num_nodes_at_locally_lowest_lvl -= 32;
    if (local_info.num_nodes_at_locally_lowest_lvl <
        CalculateLowerBoundOfOrder(local_info.locally_max_order)) {
        int8_t old_max_order = local_info.locally_max_order;
        local_info.locally_max_order =
            CalculateOrder(local_info.num_nodes_at_locally_lowest_lvl);
        //assert(old_max_order > local_info.locally_max_order);
        if (thread_id == 0) {
            if (local_info.locally_max_order >= 0)
                atomicAdd(&global_stats_per_lvl[0]
                               .sub_num_warps[local_info.locally_max_order],
                          1);
            unsigned int sub_num_warps = atomicSub(
                &global_stats_per_lvl[0].sub_num_warps[old_max_order], 1);
            //printf("sub_num_warps %d %d %d\n", 0, old_max_order, sub_num_warps-1);
        }
        if (local_info.locally_max_order == -1)
            --loop;
    }
}

__device__ __forceinline__ void UpdateWorkloadSizeAtLoopLvl(
    int thread_id, int lvl, unsigned &loop, Range &range, Range &range_cached,
    unsigned &mask_1, LocalLevelAndOrderInfo &local_info,
    StatisticsPerLvl *global_stats_per_lvl)
{
    if (mask_1 & (~(0xFFFFFFFFu << lvl)))
        return;
    if (local_info.locally_max_order < 0) {
        if (!(mask_1 & (0x1u << lvl))) {
            --loop;
        ...
        if (thread_id == 0) {
            ...
            atomicAdd(&global_stats_per_lvl[lvl].sub_num_warps[new_max_order],
                      1);
            atomicSub(
                &global_stats_per_lvl[local_info.locally_lowest_lvl].num_warps,
                1);
    ...
    if (local_info.num_nodes_at_locally_lowest_lvl <
        CalculateLowerBoundOfOrder(local_info.locally_max_order)) {
        int8_t new_max_order =
            CalculateOrder(local_info.num_nodes_at_locally_lowest_lvl);
        if (thread_id == 0) {
            if (new_max_order >= 0)
                atomicAdd(&global_stats_per_lvl[local_info.locally_lowest_lvl]
                               .sub_num_warps[new_max_order],
                          1);
            atomicSub(&global_stats_per_lvl[local_info.locally_lowest_lvl]
                           .sub_num_warps[local_info.locally_max_order],
                      1);
    ...
    LocalLevelAndOrderInfo &local_info, StatisticsPerLvl *global_stats_per_lvl)
{
    ...
    if (local_info.locally_max_order < 0) {
        ...
        local_info.num_nodes_at_locally_lowest_lvl =
            __shfl_sync(ALL_LANES, inodes_cnts, lvl);
        if (thread_id == 0) {
            atomicAdd(&global_stats_per_lvl[lvl].num_warps, 1);
    ...
    if (lvl != local_info.locally_lowest_lvl)
        return;
    local_info.num_nodes_at_locally_lowest_lvl -= 32;
    if (local_info.num_nodes_at_locally_lowest_lvl <
        CalculateLowerBoundOfOrder(local_info.locally_max_order)) {
...
__device__ __forceinline__ void InitLocalWorkloadSizeAtZeroLvl(
    int num_nodes, LocalLevelAndOrderInfo &local_info,
    StatisticsPerLvl *global_stats_per_lvl)
{
    local_info.num_nodes_at_locally_lowest_lvl =
        __shfl_sync(ALL_LANES, num_nodes, 0);
    ...
    if (local_info.num_nodes_at_locally_lowest_lvl > 0) {
        local_info.locally_max_order =
            Themis::CalculateOrder(local_info.num_nodes_at_locally_lowest_lvl);
...
__device__ __forceinline__ void InitLocalWorkloadSize(
    int lvl, int num_nodes, LocalLevelAndOrderInfo &local_info,
    StatisticsPerLvl *global_stats_per_lvl)
{
    local_info.num_nodes_at_locally_lowest_lvl =
        __shfl_sync(ALL_LANES, num_nodes, lvl);
    local_info.locally_max_order =
        Themis::CalculateOrder(local_info.num_nodes_at_locally_lowest_lvl);
    local_info.locally_lowest_lvl = lvl;
