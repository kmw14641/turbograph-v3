...
template <unsigned long long NUM_WARPS>
__forceinline__ __device__ void Get(int &idle_warp_id, WarpsStatus *warp_status,
                                    unsigned int &num_idle_warps,
                                    IdStack *id_stack)
{
    if (0 == atomicCAS(&id_stack->lock, 0, 1)) {
        ...
        if (slot > 0) {
            ...
            int slot =
                atomicAdd(&id_stack->start, (~sub) + 1) - 1;  // % NUM_WARPS;
...
template <unsigned long long NUM_WARPS>
__forceinline__ __device__ void Set(int warp_id, WarpsStatus *warp_status,
                                    unsigned int &num_idle_warps,
                                    IdStack *id_stack)
{
