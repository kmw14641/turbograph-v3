...
__forceinline__ __device__ unsigned int Pick(unsigned long long rnd,
...
__forceinline__ __device__ void Get(int warp_id, int &tgt_warp_id,
                                    ...
                                    unsigned int num_warps_which_can_push)
{
    ...
    unsigned long long rnd =
        (clock64() * 2305843009213693951u + warp_id * 2147483647) %
        137438691328u;

#ifndef LB_MODE_THEMIS
    num_warps_which_can_push = NUM_WARPS - num_idle_warps;
#endif

    if (num_warps_which_can_push > (4 * num_idle_warps) && rnd % 2 == 0)
        return;
    ...
    unsigned int num_bits2 =
        idx_bit1 == (num_bits1 - 1) ? (NUM_WARPS % 64) : 64;
...
__forceinline__ __device__ void Set(int warp_id, unsigned long long *bit1,
                                    unsigned long long *bit2)
{
    bit2 += (warp_id / 64) * 16;
    ...
    if (bits2 == 0x0ull) {
        unsigned long long sign1 = 0x1ull << (warp_id / 64);
...
// It assumes only one thread per warp executes this function
template <unsigned long long NUM_WARPS>
__forceinline__ __device__ void Get(int &tgt_warp_id, unsigned long long *bit2,
                                    unsigned int num_idle_warps)
{
    const unsigned int num_bits1 = ((NUM_WARPS - 1) / 64) + 1;
...
__forceinline__ __device__ void Set(int warp_id, unsigned long long *bit2)
{
    bit2 += (warp_id / 64);
    unsigned long long sign2 = 0x1ull << (warp_id % 64);
    unsigned long long bits2 =
        atomicCAS(bit2, 0x0000000000000000, 0x0000000000000000);
    while (true) {
        unsigned long long old_bits2 = bits2;
        bits2 = atomicCAS(bit2, bits2, sign2 | bits2);
        if (old_bits2 == bits2)
...
__forceinline__ __device__ void Get(int &idle_warp_id,
                                    unsigned int *global_num_idle_warps,
                                    unsigned int &num_idle_warps,
                                    IdStack *id_stack)
{
