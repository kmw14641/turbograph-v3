...
__global__ void BuildLevel(int *trie_pre_offsets, int *trie_keys, int *offsets,
                           int *flags, int *indirect, unsigned long long *keys,
                           int n)
{
    ...
    for (int i = thread_id; i < n; i += step) {
        if (i == 0 || (flags[i] != flags[i - 1])) {
            ...
            offsets[flags[i]] = i;
            int prefix_k = key >> 32;
            if (i == 0 || prefix_k != (keys[i - 1] >> 32))
                trie_pre_offsets[prefix_k] = flags[i];
        }
        if (i == n - 1) {
            int j = keys[i] >> 32;
...
__global__ void SetKey(unsigned long long *keys, int *flags, int *indirect,
                       int *vals, int n)
{
    int thread_id = threadIdx.x + blockDim.x * blockIdx.x;
    int step = blockDim.x * gridDim.x;
...
__device__ __forceinline__ void PrintLTS(const char *pre, int thread_id,
                                         int ts_sid_cached,
                                         Range &ts_range_cached,
                                         Range *ts_ranges)
{
    printf(
        "%s, warp_id: %d, thread_id: %d, ts_sid_cached: %d, ts_range_cached: "
        "(%d,%d), ts_ranges: (%d,%d), (%d,%d)\n",
        pre, (threadIdx.x + blockDim.x * blockIdx.x) / 32, thread_id,
        ts_sid_cached, ts_range_cached.start, ts_range_cached.end,
        ts_ranges[0].start, ts_ranges[0].end, ts_ranges[1].start,
        ts_ranges[1].end);
}
...
__device__ __forceinline__ void UpdateMaskAtIfLvlAfterPush(int lvl, int cnt,
                                                           unsigned &mask_32,
                                                           unsigned &mask_1)
{
    mask_32 =
        cnt >= 32 ? (mask_32 | (0x1 << lvl)) : (mask_32 & (~(0x1u << lvl)));
    mask_1 = mask_1 | (0x1 << lvl);
};

__device__ __forceinline__ void UpdateMaskAtZeroLvl(int lvl, int thread_id,
                                                    Range &range,
                                                    unsigned &mask_32,
                                                    unsigned &mask_1)
{
...
__device__ __forceinline__ void FillIPartAtZeroLvl(int lvl, int thread_id,
                                                   int &active, int &tid,
                                                   Range &ts_range_cached,
                                                   unsigned &mask_32,
                                                   unsigned &mask_1, int step)
{
...
{
    if (thread_id == 0)
        inodes_cnts = inodes_cnts > 32 ? inodes_cnts - 32 : 0;
    sid = ts_range_cached.start < ts_range_cached.end ? 0 : -1;
    tid = ts_range_cached.start;
    ts_range_cached.start += 32;
    ts_range_cached.start = ts_range_cached.start < ts_range_cached.end
                                ? ts_range_cached.start
                                : ts_range_cached.end;
}

__device__ __forceinline__ void UpdateMaskAtIfLvl(int lvl, int cnt,
                                                  unsigned &mask_32,
                                                  unsigned &mask_1)
{
...
__device__ __forceinline__ void FillIPartAtIfLvl(int lvl, int thread_id,
                                                 int &inodes_cnts, int &active,
                                                 unsigned &mask_32,
                                                 unsigned &mask_1)
{
    ...
    inodes_cnts -=
        (thread_id == lvl) ? ((inodes_cnts >= 32) ? 32 : inodes_cnts) : 0;
}

__device__ __forceinline__ void FillIPartAtIfLvlTest(int lvl, int thread_id,
                                                     int &cnt, int &active,
                                                     unsigned &mask_32,
                                                     unsigned &mask_1)
{
...
__device__ __forceinline__ void UpdateMaskAtLoopLvl(int lvl, Range &range,
                                                    unsigned &mask_32,
                                                    unsigned &mask_1)
{

    unsigned m = __ballot_sync(ALL_LANES, range.start < range.end);
    mask_32 = m == 0xFFFFFFFFu ? (mask_32 | (0x1 << lvl))
                               : (mask_32 & (~(0x1u << lvl)));
    mask_1 = m != 0 ? (mask_1 | (0x1 << lvl)) : (mask_1 & (~(0x1u << lvl)));
}

__device__ __forceinline__ void FillIPartAtLoopLvl(int lvl, int thread_id,
                                                   int &active, int &tid,
                                                   Range &dpart_range,
                                                   unsigned &mask_32,
                                                   unsigned &mask_1)
...
__device__ __forceinline__ void GetRangeFromTS(Range &range, Range *ts_ranges,
                                               int ts_sid_cached)
{
...
__device__ __forceinline__ unsigned PushTrapezoids(unsigned pre_lanes,
                                                   unsigned active_mask,
                                                   int &sid, int lvl,
                                                   int active, int thread_id,
                                                   int &inodes_cnts)
{
...
__device__ __forceinline__ void PushToPart(int &sid, int lvl, int active,
                                           int thread_id, int &inodes_cnts,
                                           Range &part_range,
                                           Range &local_range)
{
...
__device__ __forceinline__ bool DistributeFromPartToDPart(int thread_id,
                                                          int lvl,
                                                          int &sid_result,
                                                          Range &part_range,
                                                          Range &dpart_range)
{
    unsigned part_mask =
        __ballot_sync(ALL_LANES, part_range.start < part_range.end);
    unsigned dpart_mask =
        __ballot_sync(ALL_LANES, dpart_range.start < dpart_range.end);
    ...
    if (thread_id == 31 && sid < 32) {
        if ((range.start + 31) > dpart_range.start)
            dpart_range.end = dpart_range.start + 1;
    }

    if (sid < 32) {
        int range_end = __shfl_sync(ALL_LANES, dpart_range.end, 31);
        if (sid_result == sid)
            dpart_range.end = range_end;
        if (thread_id == sid)
            part_range.start = range_end;
    }
    if (thread_id < sid)
        part_range.start = part_range.end = 0;
    return true;
}

__device__ __forceinline__ bool DistributeFromPartToDPart(
    int thread_id, int lvl, int &sid_result, Range &part_range,
    Range &dpart_range, unsigned &mask_32, unsigned &mask_1)
{
    unsigned dpart_mask =
        __ballot_sync(ALL_LANES, dpart_range.start < dpart_range.end);
...
struct GtsRange {
    ...
    __device__ __host__ GtsRange() { start = end = 0; }
    ...
    __device__ __host__ __forceinline__ int size() { return end - start; }
...
__device__ __forceinline__ int GetLtsCnt(int thread_id, int base_sid,
                                         Range *lts_ranges)
{
...
__device__ __forceinline__ int GetNumIdleWarps(int *global_num_idle_warps,
                                               int thread_id)
{
    int num_idle_warps;
    if (thread_id == 0)
        num_idle_warps = atomicCAS(global_num_idle_warps, -1, -1);
    num_idle_warps = __shfl_sync(ALL_LANES, num_idle_warps, 0);
    return num_idle_warps;
}

__device__ __forceinline__ void CountINodesAtZeroLvl(int thread_id,
                                                     Range &range_cached,
                                                     int &num_nodes)
{
    if (thread_id == 0) {
        int step = blockDim.x * gridDim.x;
        int num_all_nodes = range_cached.start < range_cached.end
                                ? (range_cached.end - range_cached.start)
                                : 0;
...
__device__ __forceinline__ void CountINodesAtLoopLvl(int thread_id, int lvl,
                                                     Range &range_cached,
                                                     Range &range,
                                                     int &num_nodes)
{
    int cnt = range.start < range.end ? range.end - range.start : 0;
    cnt += range_cached.start < range_cached.end
               ? ((range_cached.end - range_cached.start - 1) >> 5) + 1
               : 0;
...
__device__ __forceinline__ void CountINodesAtIfLvl(int thread_id, int lvl,
                                                   int &num_nodes)
{}

__device__ __forceinline__ void PullINodesAtZeroLvlRangeStatically(
    int thread_id, Range &range_cached, int &num_nodes)
{
    ...
    if (thread_id == 0)
        num_nodes = range_cached.end - range_cached.start;
}

__device__ __forceinline__ void PullINodesAtZeroLvlStatically(
    int thread_id, Range &range_cached, int &num_nodes)
{
    int warp_id = (blockIdx.x * blockDim.x + threadIdx.x) / 32;
    if (thread_id == 0)
        range_cached.start = 32 * warp_id;
    range_cached.start =
        __shfl_sync(ALL_LANES, range_cached.start, 0) + thread_id;
    CountINodesAtZeroLvl(thread_id, range_cached, num_nodes);
    return;
}

template <int NUM_WARPS>
__device__ __forceinline__ bool PullINodesAtZeroLvlDynamically(
    int thread_id, int *global_scan_offset, int global_scan_end,
    int &local_scan_offset, Range &range_cached, int &num_nodes)
{
    num_nodes = 0;
    range_cached.start = range_cached.end + thread_id;
    if (local_scan_offset >= global_scan_end)
        return false;
    if (thread_id == 0)
        local_scan_offset = atomicAdd(global_scan_offset, 1);
    local_scan_offset = __shfl_sync(ALL_LANES, local_scan_offset, 0);
    if ((local_scan_offset >= NUM_WARPS) ||
        (local_scan_offset * 32 >= range_cached.end))
        return false;
...
__device__ __forceinline__ void PullINodesFromPPartAtZeroLvl(
    int thread_id, PushedParts::PushedPartsAtZeroLvl *gts, Range &range_cached,
    int &num_nodes)
{
    if (thread_id == 0) {
...
__device__ __forceinline__ int PushINodesToPPartAtZeroLvl(
    int thread_id, PushedParts::PushedPartsAtZeroLvl *gts, Range &range_cached,
    int step)
{
    int num_to_push;
    if (thread_id == 0) {
        int num_nodes = range_cached.start < range_cached.end
                            ? range_cached.end - range_cached.start
                            : 0;
...
__device__ __forceinline__ void PullINodesFromPPartAtLoopLvl(
    int thread_id, int lvl, PushedParts::PushedPartsAtLoopLvl *gts,
    Range &dpart_range, Range &part_range, int &inodes_cnts)
{
...
template <typename T>
__device__ __forceinline__ void PullAttributesAtLoopLvl(int thread_id,
                                                        T &dpart_attrs,
                                                        T &part_attrs,
                                                        volatile T *gts_attrs)
{
...
__device__ __forceinline__ void PullPtrIntAttributesAtLoopLvl(
    ...
    volatile int **gts_attrs)
{
    dpart_attrs = (int *)gts_attrs[thread_id];
    part_attrs = (int *)gts_attrs[thread_id + 32];
}

__device__ __forceinline__ int PushINodesToPPartAtLoopLvl(
    int thread_id, int lvl, PushedParts::PushedPartsAtLoopLvl *gts,
...
template <typename T>
__device__ __forceinline__ void PushAttributesAtLoopLvl(int thread_id,
                                                        volatile T *gts_attrs,
                                                        T &dpart_attr,
                                                        T &part_attr)
{
...
__device__ __forceinline__ void PushPtrIntAttributesAtLoopLvl(
    int thread_id, volatile int **gts_attrs, int *dpart_attr, int *part_attr)
{
    gts_attrs[thread_id] = dpart_attr;
    gts_attrs[thread_id + 32] = part_attr;
}
...
template <typename T>
__device__ __forceinline__ void PullAttributesAtIfLvl(int thread_id,
                                                      T &lts_attr,
                                                      volatile T *gts_attrs)
{
...
{
    lts_attr = (int *)gts_attrs[thread_id];
}

__device__ __forceinline__ int PushINodesToPPartAtIfLvl(
    int thread_id, int lvl, PushedParts::PushedPartsAtIfLvl *gts,
    int &inodes_cnts)
{
...
template <typename T>
__device__ __forceinline__ void PushAttributesAtIfLvl(int thread_id,
                                                      volatile T *gts_attrs,
                                                      T &lts_attr)
{
...
__device__ __forceinline__ void PushPtrIntAttributesAtIfLvl(
    int thread_id, volatile int **gts_attrs, int *&lts_attr)
{
    gts_attrs[thread_id] = lts_attr;
}
...
{
    unsigned active_mask_more_than_0 =
        __ballot_sync(ALL_LANES, inodes_cnts > 0);
    // 0010
    int num_zeros = __clz(active_mask_more_than_0);
    return 32 - num_zeros - 1;
}

__device__ __forceinline__ void ChooseLvl(int thread_id, unsigned mask_32,
                                          unsigned mask_1, int &lvl)
{
    if (mask_32 != 0) {
...
__device__ __forceinline__ void ChooseLvl(int thread_id, int inodes_cnts,
                                          int &lvl)
{
    unsigned active_mask_more_than_32 =
        __ballot_sync(ALL_LANES, inodes_cnts >= 32);
    ...
    unsigned active_mask_more_than_0 =
        __ballot_sync(ALL_LANES, inodes_cnts > 0);
    lvl =
        active_mask_more_than_0 != 0 ? __ffs(active_mask_more_than_0) - 1 : -1;
}

__device__ __forceinline__ void ChooseLvl0(int thread_id, int inodes_cnts,
                                           int &lvl)
{
...
__device__ __forceinline__ void ChooseLvl(int &lvl, int thread_id,
                                          int inodes_cnts,
                                          int num_nodes_at_loop_lvl,
                                          int loop_lvl)
{
    if (num_nodes_at_loop_lvl >= 32)
        lvl = loop_lvl;
    else
        Themis::ChooseLvl(thread_id, inodes_cnts, lvl);
}
}  // namespace Themis
