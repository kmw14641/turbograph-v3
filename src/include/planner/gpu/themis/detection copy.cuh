...
__forceinline__ __device__ void Get(int &tgt_warp_id, unsigned long long *bit1,
                                    ...
                                    unsigned int num_idle_warps)
{
    ...
    if (idx_bit1 != 0xFFFFFFFF) {
        ...
        if (bits2 & 0x8000000000000000)
            return;
        if ((bits2 & 0x7FFFFFFFFFFFFFFF) == 0)
            return;

        unsigned int num_bits2 =
            idx_bit1 == (num_bits1 - 1) ? (NUM_WARPS % 63) : 63;
        ...
        unsigned long long left_bits =
            idx2 == 0 ? shifted_bits2 : shifted_bits2 << idx2;
        unsigned long long right_bits =
            idx2 == 0 ? 0x0ull : shifted_bits2 >> (64 - idx2);
