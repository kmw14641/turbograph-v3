...
struct GpuPrimitiveType {
    ...
    GpuPrimitiveType() { cudaMalloc((void **)&val_at_device, sizeof(T)); }

    ~GpuPrimitiveType() { cudaFree(val_at_device); }
    ...
    T get() { return val_at_host; }

    void HtoD()
    {
        cudaMemcpy(val_at_device, &val_at_host, sizeof(T),
                   cudaMemcpyHostToDevice);
    }

    void DtoH()
    {
        cudaMemcpy(&val_at_host, val_at_device, sizeof(T),
...
struct GpuArrayType {
    ...
    T &operator[](size_t idx) { return val_at_host[idx]; }

    void HtoD()
    {
        cudaMemcpy(val_at_device, val_at_host, sizeof(T) * size,
                   cudaMemcpyHostToDevice);
    }

    void DtoH()
    {
        cudaMemcpy(val_at_host, val_at_device, sizeof(T) * size,
                   cudaMemcpyDeviceToHost);
    }
};
...
template <typename T>
__device__ void hashBuildUnique(unique_ht<T> *hash_table, int ht_size,
                                uint64_t hash, T *payload)
{
    ...
    while (true) {
        ...
        unsigned long long probe_hash =
            atomicCAS((unsigned long long *)&(elem->hash), HASH_EMPTY,
...
template <typename T>
__device__ bool hashProbeUnique(unique_ht<T> *hash_table, int ht_size,
                                uint64_t hash, int &numLookups, int &location)
{  //,T** payload ) {
    //int location;
...
template <typename T>
__device__ bool hashProbeUnique1(unique_ht<T> *hash_table, int ht_size,
                                 uint64_t hash, int &numLookups, T **payload)
{
    int location;
    while (numLookups < ht_size) {
        location = (hash + numLookups) % ht_size;
...
// intialize an array as used e.g. for join hash tables
__global__ void initMultiHT2(multi_ht *ht, int32_t num)
{
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num;
         i += blockDim.x * gridDim.x) {
        ht[i].offset = 0;
...
__global__ void scanMultiHT2(multi_ht *ht, uint32_t num, int *range_offset)
{
    unsigned int mask = 0xffffffff;
    int warpSize = 32;
    int lane_id = threadIdx.x % warpSize;
...
__global__ void convertMultiHTToOffset(multi_ht *ht, uint32_t num)
{
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < num;
         i += blockDim.x * gridDim.x) {
        if (i == 0)
            ...
            ht[i].offset = ht[i - 1].count;
        //if ( i == num-1 ) printf("ht[%d].offset: %d, ht[%d].count: %d\n", i, ht[i].offset, i, ht[i].count);
        //if (i < 100) printf("ht[%d].offset: %d, ht[%d].count: %d\n", i, ht[i].offset, i, ht[i].count);
    }
}
...
template <typename T>
__device__ void hashInsertMulti(multi_ht *ht, T *payload, int *range_offset,
                                int32_t ht_size, uint64_t hash, T *payl)
{
...
// join hash probe
__device__ bool hashProbeMulti(multi_ht *ht, uint32_t ht_size, uint64_t hash,
                               int &offset, int &end)
{
    uint32_t location = hash % ht_size;
    multi_ht &entry = ht[location];
...
struct OnceLock {
    ...
    {
        unsigned lockState =
            atomicCAS((unsigned *)&lock, LOCK_FRESH, LOCK_WORKING);
    ...
    __device__ void done_w() { lock = LOCK_DONE; }
...
template <typename T>
__device__ int hashAggregateGetBucket(agg_ht<T> *ht, int32_t ht_size,
                                      uint64_t grouphash, int &numLookups,
                                      T *payl)
{
    int location = -1;
    bool done = false;
...
template <typename T>
__device__ __forceinline__ int hashAggregateGetBucket(agg_ht<T> *ht,
                                                      int32_t ht_size,
                                                      uint64_t grouphash,
                                                      int &numLookups, T *payl,
                                                      int &doMemoryFence)
{
    int location = -1;
    bool done = false;
...
template <typename T>
__device__ bool hashAggregateFindBucket(agg_ht<T> *ht, int32_t ht_size,
                                        uint64_t grouphash, int &numLookups,
                                        int &location)
{
    location = -1;
...
// returns candidate bucket
template <typename T>
__device__ __forceinline__ int hashAggregateGetBucketTest(agg_ht<T> *ht,
                                                          int32_t ht_size,
                                                          uint64_t grouphash,
                                                          int &numLookups,
                                                          T *payl)
...
__host__ __device__ __forceinline__ bool trieSearch(int *keys, int *offset,
                                                    int search_key, int tid,
                                                    int &new_tid)
{
...
__device__ __forceinline__ bool binarySearch(int *array, int start, int end,
                                             int key)
{
...
__host__ __device__ __forceinline__ bool binarySearch(int *keys, int *offset,
                                                      int search_key, int tid,
                                                      int &new_tid)
{
    int start = offset[tid * 2];
...
__forceinline__ __device__ bool indexProbeMulti(const int *offsets, int tid,
                                                int &offset, int &end)
{
...
__forceinline__ __device__ void sample(int gpart_id, int thread_id,
                                       ...
                                       unsigned long long type)
{
    if (thread_id == 0) {
        ...
        */
        ...
        start = 0;
        samples[samples[0]] =
            (p > start ? p - start : 0) | (type << 60) | (smid << 52);
    }
}
