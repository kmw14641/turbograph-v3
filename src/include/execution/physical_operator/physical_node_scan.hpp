...
class PhysicalNodeScan : public CypherPhysicalOperator {
   ...
   public:
	 ...
	 * Non-schemaless APIs
	*/
    PhysicalNodeScan(Schema &sch, vector<idx_t> oids,
                     vector<vector<uint64_t>> projection_mapping,
                     std::vector<duckdb::LogicalType> scan_types,
    ...
    PhysicalNodeScan(Schema &sch, vector<idx_t> oids,
                     ...
                     std::vector<duckdb::LogicalType> scan_types,
                     vector<vector<uint64_t>> scan_projection_mapping,
                     int64_t filterKeyIndex, duckdb::Value l_filterValue,
                     duckdb::Value r_filterValue, bool l_inclusive,
                     bool r_inclusive);
	 ...
	 * Schemaless APIs
	*/
    PhysicalNodeScan(vector<Schema> &sch, Schema &union_schema,
                     vector<idx_t> oids,
                     vector<vector<uint64_t>> projection_mapping,
                     vector<vector<uint64_t>> scan_projection_mapping);
    ...
    PhysicalNodeScan(vector<Schema> &sch, Schema &union_schema,
                     ...
                     vector<duckdb::Value> &filterValues);
	...
	*/
    ...
    mutable vector<vector<uint64_t>>
        projection_mapping;  // projection mapping for operator output
    mutable vector<vector<LogicalType>> scan_types;  // types scan
    mutable vector<vector<uint64_t>>
        scan_projection_mapping;  // projection mapping for scan from the storage
