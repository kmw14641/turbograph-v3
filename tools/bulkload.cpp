...
struct BulkloadContext {
          ...
          catalog(catalog)
    {
        if (this->input_options.incremental) {
            spdlog::info(
                "[BulkloadContext] Incremental mode detected; loading existing "
                "graph catalog");
            graph_cat = (GraphCatalogEntry *)catalog.GetEntry(
                *(this->client.get()), CatalogType::GRAPH_ENTRY,
                graph_info.schema, graph_info.graph);
        }
        else {
            spdlog::info("[BulkloadContext] Creating new graph catalog");
            graph_cat = (GraphCatalogEntry *)catalog.CreateGraph(
                *(this->client.get()), &graph_info);
        }
        duckdb::SetClientWrapper(this->client,
                                 make_shared<CatalogWrapper>(catalog_wrapper));
    }
};
...
bool CreateDirectoryIfNotExists(const std::string &folder_path)
{
    try {
        if (!FolderExists(folder_path)) {
            spdlog::info("[CreateDirectory] Creating directory: {}",
                         folder_path);
            return fs::create_directories(folder_path);
        }
        spdlog::info("[CreateDirectory] Directory already exists: {}",
                     folder_path);
        return true;
    }
...
void RemoveAllFilesInDirectory(const std::string &directory)
{
    try {
        ...
        spdlog::error("[Main] Failed to clean output directory: {}. Error: {}",
                      directory, e.what());
    }
}
...
{
    return std::find(bulkload_ctx.skipped_labels.begin(),
                     bulkload_ctx.skipped_labels.end(),
                     label) != bulkload_ctx.skipped_labels.end();
}
...
void InitializeDiskAio(string &workspace)
{
    ...
    int res;
    DiskAioFactory *disk_aio_factory =
        new DiskAioFactory(res, DiskAioParameters::NUM_DISK_AIO_THREADS, 128);
    core_id::set_core_ids(DiskAioParameters::NUM_THREADS);

    spdlog::info("DiskAioParameters::NUM_DISK_AIO_THREADS: {}",
                 DiskAioParameters::NUM_DISK_AIO_THREADS);
    spdlog::info("DiskAioParameters::WORKSPACE: {}",
                 DiskAioParameters::WORKSPACE);
}
...
void CreateVertexCatalogInfos(BulkloadContext &bulkload_ctx,
                              std::string &vertex_labelset_name,
                              ...
                              PropertySchemaCatalogEntry *&property_schema_cat)
{
    string partition_name =
        DEFAULT_VERTEX_PARTITION_PREFIX + vertex_labelset_name;
    string property_schema_name =
        DEFAULT_VERTEX_PROPERTYSCHEMA_PREFIX + vertex_labelset_name;
    vector<PropertyKeyID> property_key_ids;

    spdlog::trace(
        "[CreateVertexCatalogInfos] vertex_labelset_name: {}, vertex_labels: "
        "[{}], key_names: [{}], partition_name: {}, property_schema_name: {}",
        vertex_labelset_name, join_vector(vertex_labels),
        join_vector(key_names), partition_name, property_schema_name);
    ...
    partition_cat =
        (PartitionCatalogEntry *)bulkload_ctx.catalog.CreatePartition(
            *(bulkload_ctx.client.get()), &partition_info);
    ...
    property_schema_cat =
        (PropertySchemaCatalogEntry *)bulkload_ctx.catalog.CreatePropertySchema(
            *(bulkload_ctx.client.get()), &propertyschema_info);

    spdlog::debug("[CreateVertexCatalogInfos] CreateIndexInfo");
    CreateIndexInfo idx_info(DEFAULT_SCHEMA, vertex_labelset_name + "_id",
                             IndexType::PHYSICAL_ID, partition_cat->GetOid(),
                             property_schema_cat->GetOid(), 0, {-1});
    IndexCatalogEntry *index_cat =
        (IndexCatalogEntry *)bulkload_ctx.catalog.CreateIndex(
            *(bulkload_ctx.client.get()), &idx_info);

    spdlog::debug("[CreateVertexCatalogInfos] Set up catalog informations");
    bulkload_ctx.graph_cat->AddVertexPartition(*(bulkload_ctx.client.get()),
                                               new_pid, partition_cat->GetOid(),
                                               vertex_labels);
    bulkload_ctx.graph_cat->GetPropertyKeyIDs(
        *(bulkload_ctx.client.get()), key_names, types, property_key_ids);

    partition_cat->AddPropertySchema(*(bulkload_ctx.client.get()),
                                     property_schema_cat->GetOid(),
                                     property_key_ids);
    partition_cat->SetSchema(*(bulkload_ctx.client.get()), key_names, types,
                             property_key_ids);
    ...
    property_schema_cat->SetSchema(*(bulkload_ctx.client.get()), key_names,
                                   types, property_key_ids);
    property_schema_cat->SetPhysicalIDIndex(index_cat->GetOid());
}

bool IsEdgeCatalogInfoExist(BulkloadContext &bulkload_ctx,
                            std::string &edge_type)
{
    return bulkload_ctx.catalog.GetEntry(
               *(bulkload_ctx.client.get()), CatalogType::PARTITION_ENTRY,
               DEFAULT_SCHEMA,
               DEFAULT_EDGE_PARTITION_PREFIX + edge_type) != nullptr;
}

void CreateEdgeCatalogInfos(BulkloadContext &bulkload_ctx,
                            std::string &edge_type, vector<string> &key_names,
                            vector<LogicalType> &types,
                            string &src_vertex_label, string &dst_vertex_label,
                            PartitionCatalogEntry *&partition_cat,
                            PropertySchemaCatalogEntry *&property_schema_cat,
                            IndexCatalogEntry *&adj_index_cat,
                            LogicalType edge_direction_type,
                            idx_t num_src_columns)
{
    string partition_name = DEFAULT_EDGE_PARTITION_PREFIX + edge_type;
    string property_schema_name =
        DEFAULT_EDGE_PROPERTYSCHEMA_PREFIX + edge_type;
    vector<PropertyKeyID> property_key_ids;
    ...
    if (edge_direction_type == LogicalType::FORWARD_ADJLIST) {
        ...
        property_schema_cat =
            (PropertySchemaCatalogEntry *)
                bulkload_ctx.catalog.CreatePropertySchema(
                    *(bulkload_ctx.client.get()), &propertyschema_info);

        // Create Physical ID Index Catalog & Add to PartitionCatalogEntry
        CreateIndexInfo id_idx_info(
            DEFAULT_SCHEMA, edge_type + "_id", IndexType::PHYSICAL_ID,
            partition_cat->GetOid(), property_schema_cat->GetOid(), 0, {-1});
        IndexCatalogEntry *id_index_cat =
            (IndexCatalogEntry *)bulkload_ctx.catalog.CreateIndex(
                *(bulkload_ctx.client.get()), &id_idx_info);

        // Set up catalog informations
        bulkload_ctx.graph_cat->AddEdgePartition(
            *(bulkload_ctx.client.get()), new_pid, partition_cat->GetOid(),
            edge_type);
        bulkload_ctx.graph_cat->GetPropertyKeyIDs(
            *(bulkload_ctx.client.get()), key_names, types, property_key_ids);
        ...
        vector<idx_t> src_vertex_part_cat_oids =
            bulkload_ctx.graph_cat->LookupPartition(
                *(bulkload_ctx.client.get()), {src_vertex_label},
                GraphComponentType::VERTEX);
        if (src_vertex_part_cat_oids.size() != 1)
            throw InvalidInputException(
                "The input src key corresponds to multiple vertex partitions.");
        PartitionCatalogEntry *src_vertex_part_cat_entry =
            (PartitionCatalogEntry *)bulkload_ctx.catalog.GetEntry(
                *(bulkload_ctx.client.get()), DEFAULT_SCHEMA,
                src_vertex_part_cat_oids[0]);

        vector<idx_t> dst_vertex_part_cat_oids =
            bulkload_ctx.graph_cat->LookupPartition(
                *(bulkload_ctx.client.get()), {dst_vertex_label},
                GraphComponentType::VERTEX);
        if (dst_vertex_part_cat_oids.size() != 1)
            throw InvalidInputException(
                "The input dst key corresponds to multiple vertex partitions.");
        PartitionCatalogEntry *dst_vertex_part_cat_entry =
            (PartitionCatalogEntry *)bulkload_ctx.catalog.GetEntry(
                *(bulkload_ctx.client.get()), DEFAULT_SCHEMA,
                dst_vertex_part_cat_oids[0]);
        src_vertex_part_cat_entry->GetPropertySchemaIDs(vertex_ps_cat_oids);
        bulkload_ctx.graph_cat->AddEdgeConnectionInfo(
            *(bulkload_ctx.client.get()), src_vertex_part_cat_entry->GetOid(),
            partition_cat->GetOid());
        partition_cat->SetSrcDstPartOid(src_vertex_part_cat_entry->GetOid(),
                                        dst_vertex_part_cat_entry->GetOid());
    }
    else if (edge_direction_type == LogicalType::BACKWARD_ADJLIST) {
        partition_cat = (PartitionCatalogEntry *)bulkload_ctx.catalog.GetEntry(
            *(bulkload_ctx.client.get()), CatalogType::PARTITION_ENTRY,
            DEFAULT_SCHEMA, partition_name);
        property_schema_cat =
            (PropertySchemaCatalogEntry *)bulkload_ctx.catalog.GetEntry(
                *(bulkload_ctx.client.get()),
                CatalogType::PROPERTY_SCHEMA_ENTRY, DEFAULT_SCHEMA,
    ...
    CreateIndexInfo adj_idx_info(
        DEFAULT_SCHEMA, adj_idx_name, index_type, partition_cat->GetOid(),
        property_schema_cat->GetOid(), adj_col_idx, adj_key_col_idxs);
    adj_index_cat = (IndexCatalogEntry *)bulkload_ctx.catalog.CreateIndex(
        *(bulkload_ctx.client.get()), &adj_idx_info);

    partition_cat->AddAdjIndex(adj_index_cat->GetOid());
}

void AppendAdjListChunk(
    BulkloadContext &bulkload_ctx, LogicalType edge_direction_type,
    PartitionID part_id, ExtentID max_extent_id,
    unordered_map<ExtentID, std::pair<std::pair<uint64_t, uint64_t>,
                                      vector<vector<idx_t>>>> &adj_list_buffers)
{
    ...
    spdlog::trace(
        "[AppendAdjListChunk] edge_direction_type: {}, part_id: {}, "
        "max_extent_id: {}",
        edge_direction_type.ToString(), part_id, max_extent_id);
    ...
    for (auto idx = 0; idx < max_extent_id; idx++) {
        ...
        vector<vector<idx_t>> &adj_list_buffer = iter == adj_list_buffers.end()
                                                     ? empty_adj_list
                                                     : iter->second.second;
        size_t num_adj_list =
            iter == adj_list_buffers.end() ? 0 : iter->second.first.first + 1;
        size_t adj_len_total =
            iter == adj_list_buffers.end() ? 0 : 2 * iter->second.first.second;
        ...
        tmp_adj_list_buffer.resize(slot_for_num_adj + num_adj_list +
                                   adj_len_total);
        ...
        for (size_t i = 0; i < num_adj_list; i++) {
            for (size_t j = 0; j < adj_list_buffer[i].size(); j++) {
                tmp_adj_list_buffer[slot_for_num_adj + offset + j] =
                    adj_list_buffer[i][j];
            }
        ...
        ExtentID cur_vertex_extentID =
            cur_vertex_localextentID | (((uint32_t)part_id) << 16);
        bulkload_ctx.ext_mng.AppendChunkToExistingExtent(
            *(bulkload_ctx.client.get()), adj_list_chunk, cur_vertex_extentID);
        adj_list_chunk.Destroy();
    }
}

inline void FillAdjListBuffer(
    bool load_backward_edge, idx_t &begin_idx, idx_t &end_idx, idx_t &src_seqno,
    idx_t cur_src_pid, idx_t &vertex_seqno,
    std::vector<int64_t> &dst_column_idx, vector<idx_t *> dst_key_columns,
    unordered_map<LidPair, idx_t, boost::hash<LidPair>>
        &dst_lid_to_pid_map_instance,
    unordered_map<LidPair, idx_t, boost::hash<LidPair>>
        *lid_pair_to_epid_map_instance,
    unordered_map<ExtentID, std::pair<std::pair<uint64_t, uint64_t>,
                                      vector<vector<idx_t>>>> &adj_list_buffers,
    idx_t epid_base, bool &is_target_unique, idx_t src_lid = 0)
{
    ...
    auto &adj_list = (*adj_list_buffer)[cur_src_seqno];

    if (adj_list_buffer_extent.first.first < cur_src_seqno) {
        adj_list_buffer_extent.first.first = cur_src_seqno;
    }
    if (end_idx > begin_idx) {
        uint64_t num_neighbors = end_idx - begin_idx;
        adj_list_buffer_extent.first.second += num_neighbors;
        if (((adj_list.size() / 2) + num_neighbors) > 1) {
    ...
    if (load_backward_edge) {
        if (dst_column_idx.size() == 1) {
            for (dst_seqno = begin_idx; dst_seqno < end_idx; dst_seqno++) {
                ...
                adj_list.push_back(cur_dst_pid);
                adj_list.push_back(epid_base + dst_seqno);

                // update pid pair to epid map
                pid_pair.second = cur_dst_pid;
                lid_pair_to_epid_map_instance->emplace(pid_pair,
                                                       epid_base + dst_seqno);
            }
        }
...
inline void FillBwdAdjListBuffer(
    bool load_backward_edge, idx_t &begin_idx, idx_t &end_idx, idx_t &src_seqno,
    idx_t cur_src_pid, idx_t &vertex_seqno,
    std::vector<int64_t> &dst_column_idx, vector<idx_t *> dst_key_columns,
    unordered_map<LidPair, idx_t, boost::hash<LidPair>>
...
void PopulateLidToPidMap(BulkloadContext &bulkload_ctx, std::string &label_name,
                         std::string &query, size_t num_id_columns)
{
    spdlog::trace("[PopulateLidToPidMap] Query: {}", query);
    s62_prepared_statement *prep_stmt =
        s62_prepare(const_cast<char *>(query.c_str()));
    ...
    if (num_id_columns > 2)
        throw InvalidInputException(
            "Do not support # of compound keys >= 3 currently");

    auto &lid_pid_map =
        bulkload_ctx.lid_to_pid_map
            .emplace_back(label_name,
                          unordered_map<LidPair, idx_t, boost::hash<LidPair>>())
            .second;
    ...
    while (s62_fetch_next(resultset_wrapper) != S62_END_OF_RESULT) {
        ...
        uint64_t id2 =
            (num_id_columns == 2) ? s62_get_uint64(resultset_wrapper, 2) : 0;
        lid_pid_map.emplace(LidPair(id1, id2), pid);
    }
}

void PopulateLidToPidMap(unordered_map<LidPair, idx_t, boost::hash<LidPair>>
                             *lid_to_pid_map_instance,
                         const vector<idx_t> &key_column_idxs, DataChunk &data,
                         ExtentID new_eid)
{
    idx_t pid_base = static_cast<idx_t>(new_eid) << 32;

    if (key_column_idxs.empty())
        return;

    LidPair lid_key{0, 0};
    if (key_column_idxs.size() == 1) {
        auto key_column =
            reinterpret_cast<idx_t *>(data.data[key_column_idxs[0]].GetData());
        ...
        auto key_column_1 =
            reinterpret_cast<idx_t *>(data.data[key_column_idxs[0]].GetData());
        auto key_column_2 =
            reinterpret_cast<idx_t *>(data.data[key_column_idxs[1]].GetData());
...
void ReadVertexCSVFileAndCreateVertexExtents(
    vector<LabeledFile> &csv_vertex_files, BulkloadContext &bulkload_ctx)
{
    SCOPED_TIMER_SIMPLE(ReadVertexCSVFileAndCreateVertexExtents,
                        spdlog::level::info, spdlog::level::debug);
    for (auto &vertex_file : csv_vertex_files) {
        SCOPED_TIMER_SIMPLE(ReadSingleVertexCSVFile, spdlog::level::info,
                            spdlog::level::debug);
        ...
        spdlog::info(
            "[ReadVertexCSVFileAndCreateVertexExtents] Start to load {} with "
            "label set {}",
            vertex_file_path, vertex_labelset);
        ...
        size_t approximated_num_rows =
            file_size.has_value()
                ? reader.InitCSVFile(vertex_file_path.c_str(),
                                     GraphComponentType::VERTEX, '|',
                                     file_size.value())
                : reader.InitCSVFile(vertex_file_path.c_str(),
                                     GraphComponentType::VERTEX, '|');
        ...
        spdlog::debug(
            "[ReadVertexCSVFileAndCreateVertexExtents] "
            "CreateVertexCatalogInfos");
        PartitionCatalogEntry *partition_cat;
        PropertySchemaCatalogEntry *property_schema_cat;
        CreateVertexCatalogInfos(bulkload_ctx, vertex_labelset, vertex_labels,
                                 key_names, types, key_column_idxs,
                                 partition_cat, property_schema_cat);

        spdlog::debug(
            "[ReadVertexCSVFileAndCreateVertexExtents] Initialize "
            "LID_TO_PID_MAP");
        SUBTIMER_START(ReadSingleVertexCSVFile, "InitLIDToPIDMap");
        unordered_map<LidPair, idx_t, boost::hash<LidPair>>
            *lid_to_pid_map_instance;
        if (bulkload_ctx.input_options.load_edge) {
            bulkload_ctx.lid_to_pid_map.emplace_back(
                vertex_labelset,
                unordered_map<LidPair, idx_t, boost::hash<LidPair>>());
            lid_to_pid_map_instance =
                &bulkload_ctx.lid_to_pid_map.back().second;
        ...
        spdlog::debug(
            "[ReadVertexCSVFileAndCreateVertexExtents] Start to read and "
            "create vertex extents");
        SUBTIMER_START(ReadSingleVertexCSVFile,
                       "ReadCSVFile and CreateVertexExtents");
        while (true) {
            SCOPED_TIMER_SIMPLE(ReadCSVFileAndCreateExtents,
                                spdlog::level::debug, spdlog::level::trace);

            SUBTIMER_START(ReadCSVFileAndCreateExtents, "ReadCSVFile");
            spdlog::trace(
                "[ReadVertexCSVFileAndCreateVertexExtents] ReadCSVFile");
            ...
            spdlog::trace(
                "[ReadVertexCSVFileAndCreateVertexExtents] CreateExtent");
            ExtentID new_eid = bulkload_ctx.ext_mng.CreateExtent(
                *(bulkload_ctx.client.get()), data, *partition_cat,
                *property_schema_cat);
            property_schema_cat->AddExtent(new_eid, data.size());
            SUBTIMER_STOP(ReadCSVFileAndCreateExtents, "CreateExtent");

            if (bulkload_ctx.input_options.load_edge) {
                SUBTIMER_START(ReadCSVFileAndCreateExtents,
                               "PopulateLidToPidMap");
                spdlog::trace(
                    "[ReadVertexCSVFileAndCreateVertexExtents] "
                    "PopulateLidToPidMap");
                PopulateLidToPidMap(lid_to_pid_map_instance, key_column_idxs,
                                    data, new_eid);
                SUBTIMER_STOP(ReadCSVFileAndCreateExtents,
                              "PopulateLidToPidMap");
            }
        }
        SUBTIMER_STOP(ReadSingleVertexCSVFile,
                      "ReadCSVFile and CreateVertexExtents");
        spdlog::info("[ReadVertexCSVFileAndCreateVertexExtents] Load {} Done",
                     vertex_file_path);
    }
    spdlog::debug(
        "[ReadVertexCSVFileAndCreateVertexExtents] Flush Dirty Segments and "
        "Delete From Cache");
    ChunkCacheManager::ccm->FlushDirtySegmentsAndDeleteFromcache(false);
    spdlog::info(
        "[ReadVertexCSVFileAndCreateVertexExtents] Load CSV Vertex Files Done");
}

void ReadVertexJSONFileAndCreateVertexExtents(
    vector<LabeledFile> &json_vertex_files, BulkloadContext &bulkload_ctx)
{
    SCOPED_TIMER_SIMPLE(ReadVertexJSONFiles, spdlog::level::info,
                        spdlog::level::debug);
    spdlog::info(
        "[ReadVertexJSONFileAndCreateVertexExtents] Start to load {} JSON "
        "Vertex Files",
        json_vertex_files.size());
    for (auto &vertex_file : json_vertex_files) {
        SCOPED_TIMER_SIMPLE(ReadSingleVertexJSONFile, spdlog::level::info,
                            spdlog::level::debug);

        string &vertex_labelset = std::get<0>(vertex_file);
        string &vertex_file_path = std::get<1>(vertex_file);
        spdlog::info(
            "[ReadVertexJSONFileAndCreateVertexExtents] Start to load {} with "
            "label set {}",
            vertex_file_path, vertex_labelset);

        SUBTIMER_START(ReadSingleVertexJSONFile,
                       "GraphSIMDJSONFileParser Init");
        GraphSIMDJSONFileParser reader(
            bulkload_ctx.client, &bulkload_ctx.ext_mng, &bulkload_ctx.catalog);
        if (bulkload_ctx.input_options.load_edge)
            reader.SetLidToPidMap(&bulkload_ctx.lid_to_pid_map);
        reader.InitJsonFile(vertex_file_path.c_str());
        SUBTIMER_STOP(ReadSingleVertexJSONFile, "GraphSIMDJSONFileParser Init");

        SUBTIMER_START(ReadSingleVertexJSONFile,
                       "GraphSIMDJSONFileParser LoadJSON");
        DataChunk data;
        vector<string> label_set;
        ParseLabelSet(vertex_labelset, label_set);
        reader.LoadJson(vertex_labelset, label_set, "", data,
                        bulkload_ctx.graph_cat, nullptr,
                        GraphComponentType::VERTEX);
        SUBTIMER_STOP(ReadSingleVertexJSONFile,
                      "GraphSIMDJSONFileParser LoadJSON");

        spdlog::info("[ReadVertexJSONFileAndCreateVertexExtents] Load {} Done",
                     vertex_file_path);
    }
    spdlog::debug(
        "[ReadVertexJSONFileAndCreateVertexExtents] Flush Dirty Segments and "
        "Delete From Cache");
    ChunkCacheManager::ccm->FlushDirtySegmentsAndDeleteFromcache(false);
    spdlog::info(
        "[ReadVertexJSONFileAndCreateVertexExtents] Load JSON Vertex Files "
        "Done");
}
...
void ReadVertexFilesAndCreateVertexExtents(BulkloadContext &bulkload_ctx)
{
    ...
    SeperateFilesByExtension(bulkload_ctx.input_options.vertex_files,
                             json_vertex_files, csv_vertex_files);
    spdlog::info(
        "[ReadVertexFileAndCreateVertexExtents] {} JSON Vertex Files and {} "
        "CSV Vertex Files",
        json_vertex_files.size(), csv_vertex_files.size());
    ...
    if (json_vertex_files.size() > 0) {
        ...
        ReadVertexJSONFileAndCreateVertexExtents(json_vertex_files,
                                                 bulkload_ctx);
    }
...
vector<std::string> ObtainIdColumnNames(BulkloadContext &bulkload_ctx,
                                        std::string &vertex_label)
{
    vector<idx_t> vertex_part_cat_oids =
        bulkload_ctx.graph_cat->LookupPartition(*(bulkload_ctx.client.get()),
                                                {vertex_label},
                                                GraphComponentType::VERTEX);

    if (vertex_part_cat_oids.size() != 1) {
        spdlog::error("[ObtainIdColumnNames] {} vertex partitions found",
                      vertex_part_cat_oids.size());
        throw InvalidInputException(
            "The input src key corresponds to multiple vertex partitions.");
    }
    ...
    for (size_t i = 0; i < part_id_column_idxs->size(); i++) {
        std::string id_column_name =
            part_key_names->at(part_id_column_idxs->at(i));
        spdlog::trace(
            "[ObtainIdColumnNames] part_key_names[part_id_column_idxs[{}]] = "
            "{}",
            i, id_column_name);
...
std::string ConstructLidPidRetrievalQuery(BulkloadContext &bulkload_ctx,
                                          std::string &vertex_label,
                                          vector<std::string> &id_col_names)
{
...
{
    SCOPED_TIMER_SIMPLE(ReconstructIDMappings, spdlog::level::info,
                        spdlog::level::debug);
    PrepareClient(bulkload_ctx);
    spdlog::info(
        "[ReconstructIDMappings] Start to reconstruct ID mappings for {} edge "
        "files",
        bulkload_ctx.input_options.edge_files.size());
    for (auto &edge_file : bulkload_ctx.input_options.edge_files) {
        SCOPED_TIMER_SIMPLE(ReconstructIDMappingForFile, spdlog::level::info,
                            spdlog::level::debug);
        ...
        spdlog::info(
            "[ReconstructIDMappings] Start to reconstruct ID mappings for {}",
            edge_file_path);

        if (isJSONFile(edge_file_path))
            throw NotImplementedException(
                "JSON Edge File is not supported yet");

        SUBTIMER_START(ReconstructIDMappingForFile,
                       "GraphSIMDCSVFileParser InitCSVFile");
        GraphSIMDCSVFileParser reader;
        ...
        SUBTIMER_STOP(ReconstructIDMappingForFile,
                      "GraphSIMDCSVFileParser InitCSVFile");

        auto src_it = std::find_if(
            bulkload_ctx.lid_to_pid_map.begin(),
            bulkload_ctx.lid_to_pid_map.end(),
            [&src_vertex_label](
                const std::pair<
                    string, unordered_map<LidPair, idx_t, boost::hash<LidPair>>>
                    &element) {
                return element.first.find(src_vertex_label) != string::npos;
            });

        spdlog::debug(
            "[ReconstructIDMappings] Reconstruct Src Vertex Label = {}",
            src_vertex_label);
        SUBTIMER_START(ReconstructIDMappingForFile,
                       "Reconstruct Src Vertex Label");
        if (src_it == bulkload_ctx.lid_to_pid_map.end()) {
            auto id_col_names =
                ObtainIdColumnNames(bulkload_ctx, src_vertex_label);
            std::string src_lid_pid_query = ConstructLidPidRetrievalQuery(
                bulkload_ctx, src_vertex_label, id_col_names);
            PopulateLidToPidMap(bulkload_ctx, src_vertex_label,
                                src_lid_pid_query, id_col_names.size());
        }
        else {
            spdlog::trace(
                "[ReconstructIDMappings] Mapping for Src Vertex Label = {} is "
                "found, skipped",
                src_vertex_label);
        }
        SUBTIMER_STOP(ReconstructIDMappingForFile,
        ...
        spdlog::debug(
            "[ReconstructIDMappings] Reconstruct Dst Vertex Label = {}",
            dst_vertex_label);
        SUBTIMER_START(ReconstructIDMappingForFile,
                       "Reconstruct Dst Vertex Label");
        if (dst_it == bulkload_ctx.lid_to_pid_map.end()) {
            auto id_col_names =
                ObtainIdColumnNames(bulkload_ctx, dst_vertex_label);
            std::string dst_lid_pid_query = ConstructLidPidRetrievalQuery(
                bulkload_ctx, dst_vertex_label, id_col_names);
            PopulateLidToPidMap(bulkload_ctx, dst_vertex_label,
                                dst_lid_pid_query, id_col_names.size());
        }
        else {
            spdlog::trace(
                "[ReconstructIDMappings] Mapping for Dst Vertex Label = {} is "
                "found, skipped",
                dst_vertex_label);
        }
        SUBTIMER_STOP(ReconstructIDMappingForFile,
    ...
    s62_disconnect();
    spdlog::info(
        "[ReconstructIDMappings] Reconstruct ID mappings for edge files done");
}

// For multi-threaded version, please see 6fdb44c724faf1a3bd4218a32c54e5daf6c8aeae
void ReadFwdEdgeCSVFilesAndCreateEdgeExtents(
    vector<LabeledFile> &csv_edge_files, BulkloadContext &bulkload_ctx)
{
    SCOPED_TIMER_SIMPLE(ReadFwdEdgeCSVFilesAndCreateEdgeExtents,
                        spdlog::level::info, spdlog::level::debug);
    // adj_list_buffers: <local_extent_id, 
    // <<max_src_seqno, adj_len_total>, adj_list_buffer>>
    unordered_map<ExtentID, std::pair<std::pair<uint64_t, uint64_t>,
                                      vector<vector<idx_t>>>>
        adj_list_buffers;
    spdlog::info(
        "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Start to load {} CSV Edge "
        "Files",
        csv_edge_files.size());
    for (auto &edge_file : csv_edge_files) {
        SCOPED_TIMER_SIMPLE(ReadSingleEdgeCSVFile, spdlog::level::info,
                            spdlog::level::debug);
        ...
        spdlog::info(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Start to load {} with "
            "edge type {}",
            edge_file_path, edge_type);

        if (IsEdgeCatalogInfoExist(bulkload_ctx, edge_type)) {
            spdlog::info(
                "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Edge Catalog Info "
                "for {} is already exists",
                edge_type);
        ...
        spdlog::trace("Src column name = {} (idxs = [{}])", src_vertex_label,
                      join_vector(src_column_idx));

        spdlog::trace("Dst column name = {} (idxs = [{}])", dst_vertex_label,
                      join_vector(dst_column_idx));

        // Find corresponding LID_TO_PID_MAP for source vertex label
        spdlog::debug(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] InitLIDToPIDMap");
        ...
        if (src_it == bulkload_ctx.lid_to_pid_map.end())
            throw InvalidInputException(
                "Corresponding src vertex file was not loaded");
        unordered_map<LidPair, idx_t, boost::hash<LidPair>>
            &src_lid_to_pid_map_instance = src_it->second;

        // Find corresponding LID_TO_PID_MAP for destination vertex label
        auto dst_it = std::find_if(
            bulkload_ctx.lid_to_pid_map.begin(),
            bulkload_ctx.lid_to_pid_map.end(),
            [&dst_vertex_label](
        ...
        if (dst_it == bulkload_ctx.lid_to_pid_map.end())
            throw InvalidInputException(
                "Corresponding dst vertex file was not loaded");
        unordered_map<LidPair, idx_t, boost::hash<LidPair>>
            &dst_lid_to_pid_map_instance = dst_it->second;
        SUBTIMER_STOP(ReadSingleEdgeCSVFile, "InitLIDToPIDMap");

        // Initialize PID_PAIR_TO_EPID_MAP for backward edge if needed
        spdlog::debug(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] InitLIDPairToEPIDMap");
        ...
        if (bulkload_ctx.input_options.load_backward_edge) {
            bulkload_ctx.lid_pair_to_epid_map.emplace_back(
                ...
                unordered_map<LidPair, idx_t, boost::hash<LidPair>>());
            lid_pair_to_epid_map_instance =
                &bulkload_ctx.lid_pair_to_epid_map.back().second;
            lid_pair_to_epid_map_instance->reserve(approximated_num_rows);
        }
        SUBTIMER_STOP(ReadSingleEdgeCSVFile, "InitLIDPairToEPIDMap");

        // Initialize DataChunk for reading edge file
        DataChunk data;
        data.Initialize(types, STORAGE_STANDARD_VECTOR_SIZE);

        spdlog::debug(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] CreateEdgeCatalogInfos");
        PartitionCatalogEntry *partition_cat;
        PropertySchemaCatalogEntry *property_schema_cat;
        IndexCatalogEntry *adj_index_cat;

        CreateEdgeCatalogInfos(
            bulkload_ctx, edge_type, key_names, types, src_vertex_label,
            dst_vertex_label, partition_cat, property_schema_cat, adj_index_cat,
            LogicalType::FORWARD_ADJLIST, src_column_idx.size());

        spdlog::debug(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] ClearAdjListBuffers");
        ...
        spdlog::debug(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Start to read and "
            "create edge extents");
        SUBTIMER_START(ReadSingleEdgeCSVFile,
                       "ReadCSVFile and CreateEdgeExtents");
        while (true) {
            SCOPED_TIMER_SIMPLE(ReadCSVFileAndCreateEdgeExtents,
                                spdlog::level::debug, spdlog::level::trace);

            SUBTIMER_START(ReadCSVFileAndCreateEdgeExtents, "ReadCSVFile");
            spdlog::trace(
                "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] ReadCSVFile");
            bool eof = reader.ReadCSVFile(key_names, types, data);
            ...
            spdlog::trace(
                "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] New Extent ID = {}",
                new_eid);
            ...
            for (size_t i = 0; i < src_key_columns.size(); i++) {
                src_key_columns[i] =
                    (idx_t *)data.data[src_column_idx[i]].GetData();
            }
            for (size_t i = 0; i < dst_key_columns.size(); i++) {
                dst_key_columns[i] =
                    (idx_t *)data.data[dst_column_idx[i]].GetData();
            }
            ...
            bool load_backward_edge =
                bulkload_ctx.input_options.load_backward_edge;
            if (src_column_idx.size() == 1) {
                while (src_seqno < max_seqno) {
                    ...
                    if (src_key == prev_id) {
                        ...
                        FillAdjListBuffer(
                            load_backward_edge, begin_idx, end_idx, src_seqno,
                            prev_src_pid, vertex_seqno, dst_column_idx,
                            dst_key_columns, dst_lid_to_pid_map_instance,
                            lid_pair_to_epid_map_instance, adj_list_buffers,
                            epid_base, is_target_unique);
            ...
            SUBTIMER_START(ReadSingleEdgeCSVFile,
                           "FillAdjListBuffer for Remaining");
            end_idx = src_seqno;
            FillAdjListBuffer(load_backward_edge, begin_idx, end_idx, src_seqno,
                              cur_src_pid, vertex_seqno, dst_column_idx,
                              dst_key_columns, dst_lid_to_pid_map_instance,
                              lid_pair_to_epid_map_instance, adj_list_buffers,
                              epid_base, is_target_unique);
            SUBTIMER_STOP(ReadSingleEdgeCSVFile,
                          "FillAdjListBuffer for Remaining");

            SUBTIMER_START(ReadSingleEdgeCSVFile, "CreateExtent and AddExtent");
            spdlog::trace(
                "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] CreateExtent and "
                "AddExtent");
            bulkload_ctx.ext_mng.CreateExtent(*(bulkload_ctx.client.get()),
                                              data, *partition_cat,
                                              *property_schema_cat, new_eid);
            property_schema_cat->AddExtent(new_eid, data.size());
        ...
        spdlog::debug(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Process Remaining "
            "AdjList");
        SUBTIMER_START(ReadSingleEdgeCSVFile, "Remaining AppendAdjListChunk");
        vector<idx_t> src_part_oids = bulkload_ctx.graph_cat->LookupPartition(
            *(bulkload_ctx.client.get()), {src_vertex_label},
            GraphComponentType::VERTEX);
        PartitionCatalogEntry *src_part_cat_entry =
            (PartitionCatalogEntry *)bulkload_ctx.catalog.GetEntry(
                *(bulkload_ctx.client.get()), DEFAULT_SCHEMA, src_part_oids[0]);
        AppendAdjListChunk(bulkload_ctx, LogicalType::FORWARD_ADJLIST,
                           cur_part_id, src_part_cat_entry->GetLocalExtentID(),
                           adj_list_buffers);
        SUBTIMER_STOP(ReadSingleEdgeCSVFile, "Remaining AppendAdjListChunk");

        // Set is_target_unique info on the adjacency index catalog entry
        adj_index_cat->SetIsTargetUnique(is_target_unique);
        spdlog::info(
            "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Edge type {} "
            "isTargetUnique = {}",
            edge_type, is_target_unique);
    }
    spdlog::debug(
        "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Flush Dirty Segments and "
        "Delete From Cache");
    ChunkCacheManager::ccm->FlushDirtySegmentsAndDeleteFromcache(false);
    spdlog::info(
        "[ReadFwdEdgeCSVFilesAndCreateEdgeExtents] Load CSV Edge Files Done");
}
...
void ReadFwdEdgeFilesAndCreateEdgeExtents(BulkloadContext &bulkload_ctx)
{
    ...
    SeperateFilesByExtension(bulkload_ctx.input_options.edge_files,
                             json_edge_files, csv_edge_files);
    spdlog::info(
        "[ReadEdgeFilesAndCreateEdgeExtents] {} JSON Edge Files and {} CSV "
        "Edge Files",
...
void ReadBwdEdgeCSVFilesAndCreateEdgeExtents(
    vector<LabeledFile> &csv_edge_files, BulkloadContext &bulkload_ctx)
{
    SCOPED_TIMER_SIMPLE(ReadBwdEdgesCSVFileAndCreateEdgeExtents,
                        spdlog::level::info, spdlog::level::debug);
    unordered_map<ExtentID, std::pair<std::pair<uint64_t, uint64_t>,
                                      vector<vector<idx_t>>>>
        adj_list_buffers;
    spdlog::info(
        "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Start to load {} CSV Edge "
        "Files",
        csv_edge_files.size());
    for (auto &edge_file : csv_edge_files) {
        SCOPED_TIMER_SIMPLE(ReadSingleEdgeCSVFile, spdlog::level::info,
        ...
        spdlog::info(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Start to load {} with "
            "edge type {}",
            edge_file_path, edge_type);

        if (isSkippedLabel(bulkload_ctx, edge_type)) {
            spdlog::info(
                "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Edge Catalog Info "
                "for {} is already exists",
                edge_type);
            continue;
        }
        ...
        spdlog::trace("Src vertex label = {} (idxs = [{}])", src_vertex_label,
                      join_vector(src_column_idx));

        spdlog::trace("Dst vertex label = {} (idxs = [{}])", dst_vertex_label,
                      join_vector(dst_column_idx));

        spdlog::debug(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] InitLIDToPIDMap");
        SUBTIMER_START(ReadSingleEdgeCSVFile, "InitLIDToPIDMap");
        auto src_it = std::find_if(
            bulkload_ctx.lid_to_pid_map.begin(),
            bulkload_ctx.lid_to_pid_map.end(),
        ...
        spdlog::debug(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] InitLIDPairToEPIDMap");
        SUBTIMER_START(ReadSingleEdgeCSVFile, "InitLIDPairToEPIDMap");
        auto edge_it = std::find_if(
            bulkload_ctx.lid_pair_to_epid_map.begin(),
            bulkload_ctx.lid_pair_to_epid_map.end(),
            [&edge_type](
                const std::pair<
                    string, unordered_map<LidPair, idx_t, boost::hash<LidPair>>>
                    &element) { return element.first == edge_type; });
        if (edge_it == bulkload_ctx.lid_pair_to_epid_map.end())
            throw InvalidInputException(
                "[Error] Lid Pair to EPid Map does not exists");
        unordered_map<LidPair, idx_t, boost::hash<LidPair>>
            &lid_pair_to_epid_map_instance = edge_it->second;
        SUBTIMER_STOP(ReadSingleEdgeCSVFile, "InitLIDPairToEPIDMap");
        ...
        spdlog::debug(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] CreateEdgeCatalogInfos");
        PartitionCatalogEntry *partition_cat;
        PropertySchemaCatalogEntry *property_schema_cat;
        IndexCatalogEntry *adj_index_cat;
        ...
        CreateEdgeCatalogInfos(
            bulkload_ctx, edge_type, key_names, types, src_vertex_label,
            dst_vertex_label, partition_cat, property_schema_cat, adj_index_cat,
            LogicalType::BACKWARD_ADJLIST, dst_column_idx.size());

        spdlog::debug(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] ClearAdjListBuffers");
        SUBTIMER_START(ReadSingleEdgeCSVFile, "ClearAdjListBuffers");
        ClearAdjListBuffers(adj_list_buffers);
        SUBTIMER_STOP(ReadSingleEdgeCSVFile, "ClearAdjListBuffers");
        ...
        spdlog::debug(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Start to read and "
            "create edge extents");
        SUBTIMER_START(ReadSingleEdgeCSVFile,
                       "ReadCSVFile and CreateEdgeExtents");
        while (true) {
            ...
            SUBTIMER_START(ReadCSVFileAndCreateEdgeExtents, "ReadCSVFile");
            spdlog::trace(
                "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] ReadCSVFile");
            bool eof = reader.ReadCSVFile(key_names, types, data);
            SUBTIMER_STOP(ReadCSVFileAndCreateEdgeExtents, "ReadCSVFile");
            if (eof)
                break;
            ...
            SUBTIMER_START(ReadSingleEdgeCSVFile,
                           "FillBwdAdjListBuffer for Remaining");
            spdlog::trace(
                "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] "
                "FillBwdAdjListBuffer for Remaining");
            end_idx = src_seqno;
            FillBwdAdjListBuffer(
                load_backward_edge, begin_idx, end_idx, src_seqno, cur_src_pid,
                vertex_seqno, dst_column_idx, dst_key_columns,
                dst_lid_to_pid_map_instance, lid_pair_to_epid_map_instance,
                adj_list_buffers, is_target_unique);
            SUBTIMER_STOP(ReadSingleEdgeCSVFile,
                          "FillBwdAdjListBuffer for Remaining");
        }
        SUBTIMER_STOP(ReadSingleEdgeCSVFile,
        ...
        spdlog::debug(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Process Remaining "
            "AdjList");
        SUBTIMER_START(ReadSingleEdgeCSVFile, "Remaining AppendAdjListChunk");
        vector<idx_t> src_part_oids = bulkload_ctx.graph_cat->LookupPartition(
            *(bulkload_ctx.client.get()), {src_vertex_label},
        ...
        PartitionCatalogEntry *src_part_cat_entry =
            (PartitionCatalogEntry *)bulkload_ctx.catalog.GetEntry(
                *(bulkload_ctx.client.get()), DEFAULT_SCHEMA, src_part_oids[0]);
        AppendAdjListChunk(bulkload_ctx, LogicalType::BACKWARD_ADJLIST,
                           cur_part_id, src_part_cat_entry->GetLocalExtentID(),
                           adj_list_buffers);
        SUBTIMER_STOP(ReadSingleEdgeCSVFile, "Remaining AppendAdjListChunk");
        ...
        adj_index_cat->SetIsTargetUnique(is_target_unique);
        spdlog::info(
            "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Edge type {} "
            "isTargetUnique = {}",
            edge_type, is_target_unique);
    }
    spdlog::debug(
        "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Flush Dirty Segments and "
        "Delete From Cache");
    ChunkCacheManager::ccm->FlushDirtySegmentsAndDeleteFromcache(false);
    spdlog::info(
        "[ReadBwdEdgesCSVFileAndCreateEdgeExtents] Load CSV Edge Files Done");
}
...
void ReadBwdEdgeFilesAndCreateEdgeExtents(BulkloadContext &bulkload_ctx)
{
    if (!bulkload_ctx.input_options.load_backward_edge) {
        spdlog::info(
            "[ReadBwdEdgeFilesAndCreateEdgeExtents] Skip loading backward "
            "edges");
        return;
    }
...
void ParseConfig(int argc, char **argv, InputOptions &options)
{
    ...
    desc.add_options()("help,h", "Show help message")(
        ...
        "Nodes input: <label> <file> [optional size]")(
        "relationships",
        po::value<std::vector<std::string>>()->multitoken()->composing(),
        "Relationships input: <label> <file>")(
        "relationships_backward",
        po::value<std::vector<std::string>>()->multitoken()->composing(),
        "Backward relationships input: <label> <file>")(
        "output_dir", po::value<std::string>(), "Output directory")(
        "incremental", po::value<bool>()->default_value(false),
        "Incremental load")("skip-histogram", "Skip Histogram Generation")(
    ...
    if (vm.count("nodes")) {
        ...
        while (i < nodes.size()) {
            ...
            if (i + 2 < nodes.size() &&
                std::all_of(nodes[i + 2].begin(), nodes[i + 2].end(),
                            ::isdigit)) {
    ...
    if (vm.count("relationships")) {
        ...
        while (i < relationships.size()) {
            ...
            if (i + 2 < relationships.size() &&
                std::all_of(relationships[i + 2].begin(),
                            relationships[i + 2].end(), ::isdigit)) {
    ...
    if (vm.count("relationships_backward")) {
        auto relationships_backward =
            vm["relationships_backward"].as<std::vector<std::string>>();
        size_t i = 0;
        while (i < relationships_backward.size()) {
            ...
            if (i + 2 < relationships_backward.size() &&
                std::all_of(relationships_backward[i + 2].begin(),
                            relationships_backward[i + 2].end(), ::isdigit)) {
    ...
    if (vm.count("incremental")) {
        ...
        if (options.incremental && options.vertex_files.size() > 0) {
            throw InvalidInputException(
                "Incremental load only supports edge label");
        }
    }
    ...
    spdlog::info("[ParseConfig] Load Backward Edge: {}",
                 options.load_backward_edge);
...
int main(int argc, char **argv)
{
    ...
    if (!options.incremental)
        RemoveAllFilesInDirectory(options.output_dir);
    CatalogManager::CreateOrOpenCatalog(options.output_dir);
    InitializeDiskAio(options.output_dir);

    ChunkCacheManager::ccm =
        new ChunkCacheManager(options.output_dir.c_str(), options.standalone);
    std::unique_ptr<DuckDB> database =
        make_unique<DuckDB>(options.output_dir.c_str());
    ...
    BulkloadContext bulkload_context{
        ...
        database->instance->GetCatalogWrapper(), graph_info};
    ...
    try {
        SCOPED_TIMER_SIMPLE(Bulkload, spdlog::level::info,
                            spdlog::level::debug);
        ...
        spdlog::error("[Main] Caught system_error with code [{}] meaing [{}]",
                      e.code().value(), e.what());
    }
    ...
    ChunkCacheManager::ccm->FlushMetaInfo(
        bulkload_context.input_options.output_dir.c_str());
